...
final class Updater: NSObject, SPUUpdaterDelegate, SPUUserDriver {
    ...
    /// once a scheduled check has found one — both bring the update Sparkle
    /// already has into focus rather than checking again. And the answer to
    /// the one question the app asks on its second launch, kept where it can
    /// be changed: off means the app makes no request at all, and the item
    ...
    @objc private func checkFromMenu() { checkForUpdates() }
    @objc private func toggleAutomaticFromMenu() { automaticChecks.toggle() }

    /// The mark on the icon while an update waits. Not one of the menu bar's
    /// own badge states: it says nothing about what the app is doing, and it
    /// has to be visible through all of them, listening included, or it is
    /// visible only when nothing is playing. So it is a second mark at the
    /// top *left*, the corner the health badge leaves free.
    ///
    /// Red, with a count in it — and red is the colour MenuBar.swift says
    /// never to use. The exception holds because this is not a dot: a red
    /// disc with a white 1 in it is the shape every notification badge on the
    /// Mac has, and it reads as "one thing waiting for you", not as a state
    /// light. A plain red dot in this corner would read as recording, and a
    /// teal one, tried first, was simply not noticed.
    private lazy var badge = CountBadgeView(count: 1)

    func decorate(_ button: NSStatusBarButton, glyph: NSRect) {
