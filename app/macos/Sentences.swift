...
struct Sentence: Equatable {
    ...
    var id: String { "\(start.rounded(toPlaces: 2))|\(text)" }
}
...
struct SentenceBuffer {
    ...
    mutating func ingest(_ words: [TimedWord], ended: Bool,
...
final class TranslationPipeline {
    ...
    /// Translated words, the speculative tail, and the audio times each settled
    /// chunk begins at. The overlay uses the last of these to keep a clause on
    /// screen across a page break.
    private let onTranslated: ([TimedWord], String, [TimeInterval]) -> Void
        ...
        Task { [weak self] in
            ...
            do {
                ...
                for (sentence, text) in zip(batch, results) {
                    self.done[sentence.id] = Self.matchLeadingCase(text, to: sentence.text)
                }
                ...
                if let sending, sending == self.tail, results.count == texts.count {
                    self.translatedTail = Self.matchLeadingCase(
    ...
    private func emit() {
        ...
        var speculative = translatedTail
        // `Pill.attributed` concatenates the settled and tentative runs with no
        // separator: the core's own tentative text arrives carrying its leading
        // space, so nothing downstream adds one. Ours is a freshly translated
        // sentence with no such convention, and without this it butts straight
        // against the last settled word.
        if !out.isEmpty, !speculative.isEmpty { speculative = " " + speculative }
        guard !out.isEmpty || !speculative.isEmpty else { return }
        onTranslated(out, speculative, starts)
    }
    ...
    private static func matchLeadingCase(_ translated: String, to source: String) -> String {
