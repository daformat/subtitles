...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    /// an edition of the app adds to this menu — an updater's items, a
    /// licence's — arrives through these rather than as code of its own here,
    /// so this file has no opinion about which edition it is in. Under Pause
    /// is where news goes; above Settings is where a thing about this copy of
    /// the app, rather than about captions, belongs. Called on every rebuild,
    /// so the titles can say what is true now.
    var itemsUnderPause: () -> [NSMenuItem] = { [] }
    var itemsAboveSettings: () -> [NSMenuItem] = { [] }
    /// A second mark on the icon, from outside, called on every refresh with
    /// the button and the rect the glyph occupies in it. The health badge
    /// owns the top right (`badgeRect`); the top left is the caller's.
    var decorateStatusButton: ((NSStatusBarButton, NSRect) -> Void)?
    ...
    func updateHealthIndicator() {
        ...
        refreshIcon()
        decorateStatusButton?(button, glyphRect(in: button))
    ...
    /// Where the glyph sits inside the button. The button is wider and taller
    /// than the icon it draws, so anything placed against the button's own
    /// bounds parks in the padding instead of on the icon.
    private func glyphRect(in button: NSStatusBarButton) -> NSRect {
        let glyph = button.image?.size ?? NSSize(width: 16, height: 16)
        return NSRect(x: (button.bounds.width - glyph.width) / 2,
                      y: (button.bounds.height - glyph.height) / 2,
                      width: glyph.width, height: glyph.height)
    }

    /// Top-right corner of the glyph, for the health badge.
    private func badgeRect(in button: NSStatusBarButton, size: CGFloat) -> NSRect {
        let glyphRect = glyphRect(in: button)
    ...
    private func rebuild() {
        ...
        // Straight under Pause, in a group of their own: news goes at the top.
        let news = itemsUnderPause()
        if !news.isEmpty {
            news.forEach(menu.addItem)
            menu.addItem(.separator())
        }
        ...
        menu.addItem(.separator())
        itemsAboveSettings().forEach(menu.addItem)
