...
// looks like a browser playing a video. And sound is not words: a music player
// is playing in every sense, and none of the words on screen are its.
//
...
//    (`Rules.takeover`), or
//  - by ear: someone listened to it for a moment and heard it saying the
//    words on screen (`judge`), which the monitor does with a short-lived tap
//    on that app when the picker names it as a `candidate`.
//
// Either way an app that started after the pick and has earned it takes over.
// One heard to be silent never does, and neither does one heard to be making
// sound with no voice in it, however long it plays: whatever the words on
// screen are, they are not its. Both are listened to again after a while,
// since a burst of dings can become a call, and so can hold music. When the
// pick stops, the icon falls to the best of the rest — heard speaking before
// earned on time, earned before not, and the most recently started among
// equals, since the one that just began is the likelier voice — but never to
// an app heard to be silent. With nothing left at all the last pick is kept:
// the words on screen came from somewhere, and that is the best guess there
// is.
//
...
public struct PlayingAppPicker: Equatable {
    ...
    public struct Rules: Equatable {
        ...
        /// Seconds after a verdict short of a voice before an app still
        /// playing is listened to again: a burst of dings can become a call,
        /// and so can hold music.
        ...
        /// and nor does one heard to be playing sound without a voice.
        ///
    ...
    /// What listening to an app found in it.
    public enum Heard: Equatable {
        /// Nothing, or as good as: a ding's tail, a stream playing zeros.
        case silence
        /// Sustained sound that the words on screen are not coming from:
        /// music, a game, a film's score, and a voice other than the one
        /// captioned, a song's lyrics under a call.
        case sound
        /// The words on screen, or a voice where that cannot be told.
        case speech
    }

    /// What listening to an app found, and when.
    ...
    /// heard — or was heard to be something short of a voice long enough ago
    /// to be worth a second listen. The most recently started first, since
    /// that is the decision at hand.
    ///
    ...
    public func candidate() -> String? {
        ...
        let due = playing.filter { app in
            ...
            return verdict.heard != .speech && now - verdict.at >= rules.recheck
        }
    ...
    @discardableResult
    public mutating func judge(_ app: String, heard: Heard, at now: TimeInterval) -> String? {
        guard since[app] != nil else { return pick }
        verdicts[app] = Verdict(heard: heard, at: now)
        self.now = now
        decide()
        return pick
    }
    ...
    /// Heard speaking (3), earned the icon on time (2), not yet or heard
    /// making sound with no voice in it (1), or heard to be silent (0). Only
    /// a voice earns the icon by ear: sound without one counts for exactly as
    /// much as nothing heard yet, so a music player takes over neither by ear
    /// nor, whatever its age, by time. The pick is never listened to while it
    /// is the pick, and a silent app never becomes the pick, so the pick is
    /// never 0.
    private func rank(_ app: String) -> Int {
        if let verdict = verdicts[app] {
    ...
    private mutating func decide() {
        if let pick, since[pick] != nil {
            ...
            // it, or has earned it just as well and started later — the
            // newcomer. Two apps that started together stay with whichever
            // was picked first.
            guard let other = best(of: playing.filter { $0 != pick }) else { return }
            if rank(other) > rank(pick)
                || (rank(other) == rank(pick) && rank(other) >= 2 && since[other]! > since[pick]!) {
                self.pick = other
            }
            return
        }
